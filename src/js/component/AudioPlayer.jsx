...
const AudioPlayer = () => {
    ...
    return (
        <div>
            {domList}
            <audio ref={songRef} src={songRef.src} autoPlay onPlay={() => setPlay(true)} onPause={() => setPlay(false)}
                onEnded={() => songEnded()} onLoadedMetadata={() => setDuration(Math.round(songRef.current.duration))} />
